    ...
    <section>
        <div class="container pt150 pb60">
            ...
            <div class="flex flex--center">
                <div class="col-md-8">
                    <form method="POST" action="{{ route('dogodki.store') }}" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="flex flex--middle flex--center pb20">
                            <p class="mb0">Navijaško središče</p>
                            <label class="switch flex flex--middle ml10 mr15">
                                <input type="checkbox" name="type" value="slo">
                                <span></span>
                            </label>
                            <p class="mb0">Slovensko središče</p>
                        </div>
                        <input type="number" value="{{ old('price') }}" name="price" placeholder="Cena dogodka"
                               class="box mb15">
                        <input type="datetime" value="{{ old('begins_at') }}" name="begins_at"
                               placeholder="Datum dogodka" class="box mb15">
                        <div class="flex flex--center">
                        ...
                        <div>
                            <div id="slo">
                                <h4>Slo</h4>
                                <input value="{{ old('name.sl') }}" name="name[sl]"
                                       placeholder="Ime dogodka (slo)" class="box mb15">
                                <textarea class="box mb15" name="excerpt[sl]"
                                          placeholder="kratek opis (slo)">{{ old('excerpt.sl') }}</textarea>
                                <textarea class="box mb15" name="description[sl]"
                                          placeholder="opis (slo)">{{ old('description.sl') }}</textarea>
                            </div>
                            <div id="eng" style="display: none">
                                <h4>Eng</h4>
                                <input value="{{ old('name.en') }}" name="name[en]"
                                       placeholder="Ime dogodka (eng)" class="box mb15">
                                <textarea class="box mb15" name="excerpt[en]"
                                          placeholder="kratek opis (eng)">{{ old('excerpt.en') }}</textarea>
                                <textarea class="box mb15" name="description[en]"
                                          placeholder="opis (eng)">{{ old('description.en') }}</textarea>
                            </div>
                            <div id="ger" style="display: none">
                                <h4>Ger</h4>
                                <input name="name[de]" value="{{ old('name.de') }}" placeholder="Ime dogodka (ger)"
                                       class="box mb15">
                                <textarea class="box mb15" name="excerpt[de]"
                                          placeholder="kratek opis (ger)">{{ old('excerpt.de') }}</textarea>
                                <textarea class="box mb15" name="description[de]"
                                          placeholder="opis (ger)">{{ old('description.de') }}</textarea>
                            </div>
                        </div>
